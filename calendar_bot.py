...
# بررسی ارسال قبلی
# =========================================================
def already_sent(jalali_date):
    history = load_history()
    if history.get("last_sent_date") == jalali_date:
        print(
            "Calendar already sent for:",
            jalali_date
        )
        return True
    return False
...
def main():
    ...
    # =====================================================
    if already_sent(jalali_date):
        print(
            "Duplicate calendar detected."
        )
        print(
...
""".strip()
    ...
    # ارسال
    # =====================================================
    success = send_calendar(
        image_url,
        message
    )
    # =====================================================
    # ثبت ارسال فقط در صورت موفقیت
    # =====================================================
    if success:
        history = load_history()
        history["last_sent_date"] = jalali_date
        history["last_sent_at"] = (
            iran_time.isoformat()
        )
        if save_history(history):
            print(
                "Calendar history saved."
            )
        else:
            print(
                "WARNING: Calendar sent but history could not be saved."
            )
    else:
        print(
            "Calendar was NOT sent successfully."
        )
        print(
